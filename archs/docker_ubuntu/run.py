...
def main(*args, **kwargs):
    ...
    from run.archs.octavia.config import get_config as octavia_get_config
    ...
    #TODO: Set up DOCKER infrastructure (template Dockerfile, copy etc)
    #TODO: Run DOCKER so can connect via SSH, share correct volumes etc
    ...
    from run.archs.arc.config import get_config as arc_get_config
    arc_cfg = arc_get_config()

    venv_remote_path = arc_cfg["venv_path"]
    exp_root_remote_path = os.path.join(arc_cfg["experiment_root_path"], "cmap", setup, tag)
    lib_remote_path = arc_cfg["lib_path"]
    data_remote_path = arc_cfg["data_path"]
    ...
    def setup_ssh():
        ...
        arch_cfg["config_path"] = os.path.join(exp_root_remote_path, "config").format(arch_cfg, tag)
        arch_cfg["logs_path"] = os.path.join(exp_root_remote_path, "log")
        ...
        arch_cfg["visdom_options"] = dict( host = "octavia.robots.ox.ac.uk",
                                           port = 8097)
        ...
        load_modules = ""
        run_application = "python3 {}/run.py {} {}".format(os.path.join(exp_root_remote_path, "src"),
                                                        os.path.join(arch_cfg["config_path"], "arch.yaml"),
                                                        os.path.join(arch_cfg["config_path"], "setup.yaml"))
        venv_name = "slamdoom"
        ...
        setup_module.write(config_local_path, arch_cfg["logs_path"])
        # run("mkdir -p {}".format(, ))
        put(os.path.join(config_local_path, "config.yaml"), os.path.join(arch_cfg["config_path"], "config.yaml"))


    execute(setup_ssh, hosts=["exet2727@arcus-b.arc.ox.ac.uk"])
